...
int fill_with_randoms(int v[], int n, int a, int b) {
	for (int i = 0; i < n; i++) {
		v[i] = (rand() % (b - a + 1)) + a;
	}
}
...
void mean_variance(const int v[], int n, double* mean, double* variance) {
	...
	*mean = round( *mean / (double)n *100.00)/100.00;
	for (int i = 0; i < n; i++) {
		*variance += (v[i] - *mean) * (v[i] - *mean);
	}
	*variance = round(*variance / (double)n * 100.00) / 100.00;
}
